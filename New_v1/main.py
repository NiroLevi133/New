...
try:
    ...
    async def export_results(data: dict):
        ...
        try:
            ...
            for result in results:
                ...
                relevant_fields = extract_relevant_guest_details(pd.Series(guest_details))
                row_data.update(relevant_fields)
    ...
    # WebSocket support for real-time updates (אופציונלי)
    @app.websocket("/ws/{phone}")
    async def websocket_endpoint(websocket, phone: str):
        """WebSocket עבור עדכונים בזמן אמת"""
        await websocket.accept()
        try:
            while True:
                # שלח עדכונים בזמן אמת על התקדמות העיבוד
                user_data = await get_user_data(phone)
                await websocket.send_json({
                    "type": "status_update",
                    "daily_matches_used": user_data["daily_matches_used"],
                    "progress": user_data["current_progress"],
                    "timestamp": datetime.datetime.now().isoformat()
                })
                await asyncio.sleep(30)  # עדכון כל 30 שניות
        except Exception as e:
